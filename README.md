...
[VR-Stick-Scope-v1.0.5-public.zip](https://github.com/Oinari-Sama/VR-STICK-SCOPE-PUBLIC/releases/download/v1.0.5-public/VR-Stick-Scope-v1.0.5-public.zip)
...
診断時は SteamVR と VRChat を起動し、VRChat が入力を受け付ける状態で操作してください。
...
4. SteamVR と VRChat を起動します。
5. VRChat 上で、スティック入力で移動や旋回が反応することを確認します。
...
VRChat で補正を試す場合は、先に VRChat 側で OSC を有効にしてください。VRChat は起動したまま、入力を受け付ける状態にします。その後、VR Stick Scope の `出力設定` で `VRChatへ補正入力を送る` を押します。
